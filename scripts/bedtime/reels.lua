...
local function play(state)
  ...
  elseif state == 0 then 
    ...
    play_counter:stop()
    for i=1,TR do
    if reel.rec[i] == 1 then rec(i,0) end
      engine.stop(i)
      reel.play[i] = 0
    end
  end
    ...
    for i=1,TR do
      if reel.name[i] ~= "-" then
        if reel.name[i]:find("*") then
          local name = reel.name[i] == "*-" and (txt .. "rec-" .. i) or reel.name[i]:sub(2,-1) -- remove asterisk
          local save_path = audio_dir .."reels/" .. name .. ".aif"
          reel.paths[i] = save_path
          print("saving "..i .. "clip at " .. save_path, reel.s[i],reel.e[i])
          engine.write(audio_dir.."reels/" .. name ..".aif",reel.s[i],reel.e[i])
        end
      end
    end
    tab.save(reel, audio_dir.."reels/".. txt ..".reel") --[[name:match("[^.]*")]]
  else
    print("save cancel")
  end
  filesel = false
    ...
    elseif n == 3 then
      ...
      elseif settings == true then
        ...
        elseif settings_list.index == 6 then
          ...
          loop(reel.loop == 1 and reel.loop or 0)
