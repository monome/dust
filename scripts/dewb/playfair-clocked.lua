...
local externalClockPPQ = 24
local externalClockTicks = externalClockPPQ - 1
...
function init()
  ...
  screen.line_width(1)
  params:add_option("clock", {"internal", "external"}, 1)
  params:set_action("clock", function(v) if v == 1 then externalClockTicks = externalClockPPQ - 1 end end)
...
function clock_external()
  if params:get("clock") == 2 then
    externalClockTicks = (externalClockTicks + 1) % externalClockPPQ
    if externalClockTicks % (externalClockPPQ/4) == 0 then
      step()
    elseif externalClockTicks == 1 then -- erase beat indicator after tick 0
      redraw()
    end
  end
end

function clock_internal()
...
function key(n,z)
  ...
  elseif n==2 and z==1 then 
    reset = true
    externalClockTicks = externalClockPPQ - 1
